...
double trapezoid_method() {
	...
	while (fabs(integral_trapezoid_h2 - integral_trapezoid_h) >= (3 * eps1)){
...
double Simpsons_method() {
	double h = (b - a) / (2 * n);
	int m = n / 2;
	double integral_Simpsons_h = (function_value(a) + function_value(b)) * h / 3;
	double integral_Simpsons_h2 = (function_value(a) + function_value(b)) * h / 2 / 3;
	
	while (fabs(integral_Simpsons_h2 - integral_Simpsons_h) >= (15 * eps1)) {
